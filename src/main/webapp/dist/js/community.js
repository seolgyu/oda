...
function checkCommunityName() {
    ...
    // 두 JSP의 에러 ID가 달라도 작동하도록 둘 다 선택
    ...
    ajaxRequest('checkName', 'GET', { com_name: name, community_id: communityId }, 'json', function(data) {
        if (data.isDuplicate) {
...
function submitCommunity($form) {
    ...
    ajaxRequest(mode, 'POST', params, 'json', function(res) {
        if (res.status === "success") {
            alert(mode === 'update' ? "수정되었습니다." : "생성되었습니다.");
            
            location.href = "main?community_id=" + (res.community_id || res.dto.community_id);
        } else {
            alert("처리 실패: " + (res.message || "데이터 확인 후 다시 시도해주세요."));
        }
    });
}

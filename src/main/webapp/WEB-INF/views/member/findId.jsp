...
<body>
			...
			<div class="d-flex justify-content-center align-items-center w-100"
				...
				<div
					...
					<div class="position-relative">
						<div
							...
							<span class="material-symbols-outlined text-white"
								style="font-size: 3rem;">person_search</span>
						</div>
						<div
					...
					<div class="text-center mb-2">
						...
						<p class="text-secondary text-xs mb-0" style="font-size: 0.85rem;">Enter
							your name and email to recover</p>
					</div>
					...
					<div id="findIdResult" class="w-100 p-3 text-center fade-in"
						style="border-radius: 0.75rem; font-size: 0.9rem; display: none; transition: all 0.3s;">

						<div id="resultText" class="fw-medium"></div>

						<div>
							<span id="retryBtn" class="text-decoration-underline"
								style="cursor: pointer; font-size: 0.75rem; opacity: 0.7; color: inherit; display: none;"
								onclick="location.reload();"> Try Again </span>
						</div>
					</div>
	...
	<script type="text/javascript">
		function sendFindId() {
		    ...
		    const $submitBtn = $("button[onclick='sendFindId();']");
			...
			$.ajax({
				type : "POST",
				url : "${pageContext.request.contextPath}/member/findIdAjax",
				data : {userName: userName, email: email},
				dataType : "json",
				success : function(data) {
					if (data.status === "success") {
		                showResult("Found Your ID: <b class='text-white'>" + data.userId + "</b>", "success");
		                $("#retryBtn").show();
		                
		                $submitBtn
		                    .text("GO TO LOGIN")
		                    .css("background", "linear-gradient(to right, #10b981, #34d399)") // 초록색 계열
		                    .prop("disabled", false)
		                    .attr("onclick", "location.href='${pageContext.request.contextPath}/member/login'");
